...
fix_reports_for_clients <- function(clients) {
  for(client_name in clients) {
    connections <- database$get_connections(client_name)
    reports <- connections$customer$get("SELECT * FROM insight")
    if(nrow(reports) > 0) {
      data <- Data$connect(connections$customer)
      check_old_reports(reports)
      # reports_with_count_percentage <<- check_reports_to_edit(reports, edit = F)
      # print(paste0("For client ", client_name, " reports with count_percentage are: ", reports_with_count_percentage))
    }
  }
}

check_reports_to_edit <- function(reports, edit = FALSE) {
